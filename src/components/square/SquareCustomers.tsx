...
export default function SquareCustomers() {
  ...
  const fetchCustomers = async () => {
    ...
    setError(null);
    try {
      const data = await SquareApiService.listCustomers({ limit: 100 });
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="bg-white dark:bg-slate-800 rounded-lg shadow dark:shadow-slate-900/50 p-6">
        <h2 className="text-xl font-semibold text-gray-900 dark:text-white mb-4">Square Customers</h2>
      ...
      {filteredCustomers.length > 0 ? (
          ...
          <table className="min-w-full divide-y divide-gray-200 dark:divide-slate-700">
            <thead className="bg-gray-50 dark:bg-slate-900">
              <tr>
                ...
                <th className="px-6 py-3 text-left text-xs font-medium text-gray-500 dark:text-gray-400 uppercase">Customer ID</th>
              </tr>
            </thead>
            <tbody className="bg-white dark:bg-slate-800 divide-y divide-gray-200 dark:divide-slate-700">
              {filteredCustomers.map((customer, index) => (
                <tr key={index} className="hover:bg-gray-50 dark:hover:bg-slate-700">
                  ...
                  <td className="px-6 py-4 text-sm font-mono text-gray-600 dark:text-gray-400 text-xs">{customer.id}</td>
                </tr>
              ))}
            </tbody>
          </table>
        ...
        <div className="bg-gray-50 dark:bg-slate-800 border border-gray-200 dark:border-slate-700 rounded-lg p-8 text-center">
          <p className="text-gray-600 dark:text-gray-400">
            {customers.length === 0
              ? 'No customers to display. Click "Fetch Data" to load from Square.'
