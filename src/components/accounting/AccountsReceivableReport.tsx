...
export default function AccountsReceivableReport({ onNavigateToSettings, onNavigateToCustomer }: AccountsReceivableReportProps) {
  ...
  const loadData = async () => {
    ...
    try {
      ...
      // Use status_stage instead of Printavo status to determine AR eligibility
      // This ensures refunded invoices appear correctly in AR
      let query = supabase
      ...
      <div className="grid grid-cols-1 md:grid-cols-5 gap-4">
        <div className="bg-white dark:bg-slate-800 rounded-lg shadow p-6">
          <div className="flex items-center justify-between mb-2">
            <span className="text-sm font-medium text-gray-600 dark:text-gray-400">Total Outstanding</span>
          ...
          <div className="text-xs text-gray-500 dark:text-gray-400 mt-1">{invoices.length} invoices</div>
        </div>

        {agingBuckets.map((bucket, index) => (
          <div key={bucket.bucket} className="bg-white dark:bg-slate-800 rounded-lg shadow p-6">
            <div className="flex items-center justify-between mb-2">
              <span className="text-sm font-medium text-gray-600 dark:text-gray-400">{bucket.bucket} Days</span>
              <AlertCircle className={`w-5 h-5 ${index === 3 ? 'text-red-600 dark:text-red-500' : index === 2 ? 'text-orange-600 dark:text-orange-500' : 'text-yellow-600 dark:text-yellow-500'}`} />
            </div>
            <div className="text-2xl font-bold text-gray-900 dark:text-white">
              ${bucket.total.toLocaleString('en-US', { minimumFractionDigits: 2, maximumFractionDigits: 2 })}
            </div>
            <div className="text-xs text-gray-500 dark:text-gray-400 mt-1">{bucket.count} invoices</div>
          </div>
        ))}
      </div>
        ...
        <div className="overflow-x-auto">
          <table className="w-full">
            <thead className="bg-gray-50 dark:bg-slate-900 border-b border-gray-200 dark:border-slate-700">
              <tr>
                ...
                <th className="px-6 py-3 text-right text-xs font-medium text-gray-500 dark:text-gray-400 uppercase tracking-wider">Balance Remaining</th>
                <th className="px-6 py-3 text-center text-xs font-medium text-gray-500 dark:text-gray-400 uppercase tracking-wider">Aging Bucket</th>
              </tr>
            </thead>
                ...
                <tr key={invoice.invoice_id} className="hover:bg-gray-50 dark:hover:bg-slate-700">
                  ...
                  <td className="px-6 py-4 whitespace-nowrap text-center">
                    <span className={`inline-flex px-2 py-1 text-xs font-semibold rounded-full ${
                      invoice.aging_bucket === '90+' ? 'bg-red-100 dark:bg-red-900/30 text-red-800 dark:text-red-400' :
                      invoice.aging_bucket === '61-90' ? 'bg-orange-100 dark:bg-orange-900/30 text-orange-800 dark:text-orange-400' :
                      invoice.aging_bucket === '31-60' ? 'bg-yellow-100 dark:bg-yellow-900/30 text-yellow-800 dark:text-yellow-400' :
          ...
          {filteredInvoices.length === 0 && (
            <div className="text-center py-12">
              ...
              <h3 className="text-lg font-semibold text-gray-900 dark:text-white mb-2">No Outstanding Invoices</h3>
              <p className="text-gray-600 dark:text-gray-400">All invoices have been paid.</p>
            </div>
          )}
        </div>
      </div>
