  ...
  return (
    <div className="space-y-6">
      <div className="flex items-center justify-between">
        <div>
          <h2 className="text-2xl font-bold text-gray-900 dark:text-white">Production Dashboard</h2>
          <p className="text-sm text-gray-600 dark:text-gray-400 mt-1">Manage orders through your production workflow</p>
        </div>
      ...
      {loading ? (
        <div className="bg-white dark:bg-slate-800 rounded-lg shadow-sm border border-gray-200 dark:border-slate-700 p-12 text-center">
          ...
          <p className="text-gray-600 dark:text-gray-400">Loading production board...</p>
        </div>
      ) : stages.length === 0 ? (
        <div className="bg-white dark:bg-slate-800 rounded-lg shadow-sm border border-gray-200 dark:border-slate-700 p-12 text-center">
          <h3 className="text-lg font-medium text-gray-900 dark:text-white mb-2">No workflow stages configured</h3>
          <p className="text-gray-600 dark:text-gray-400 mb-4">Set up your production workflow to get started</p>
        ...
        <div className="flex gap-4 overflow-x-auto pb-4">
          {stages.map((stage) => {
            ...
            return (
              <div
                key={stage.id}
                className="flex-shrink-0 w-80 bg-white dark:bg-slate-800 rounded-lg shadow-sm border border-gray-200 dark:border-slate-700"
                onDragOver={handleDragOver}
                onDrop={() => handleDrop(stage.id)}
              >
                <div className="p-4 border-b border-gray-200 dark:border-slate-700" style={{ backgroundColor: `${stage.color}10` }}>
                  <div className="flex items-center justify-between">
                  ...
                  {stageOrders.length === 0 ? (
                    ...
                    stageOrders.map((order) => (
                      <div
                        ...
                        className="bg-white dark:bg-slate-700 border border-gray-200 dark:border-slate-600 rounded-lg p-4 hover:shadow-md transition-shadow cursor-move"
                      >
                        <div className="flex items-start justify-between mb-2">
                          <h4 className="font-medium text-gray-900 dark:text-white text-sm">{order.orderNumber}</h4>
                        ...
                        {order.tags.length > 0 && (
                          <div className="flex flex-wrap gap-1 mt-2">
                            {order.tags.map((tag, index) => (
                              <span
                                key={index}
                                className="inline-flex items-center gap-1 px-2 py-1 bg-gray-100 dark:bg-slate-600 text-gray-700 dark:text-gray-300 rounded text-xs"
                              >
                        ...
                        <div className="mt-3 pt-3 border-t border-gray-100 dark:border-slate-600 flex items-center justify-between">
