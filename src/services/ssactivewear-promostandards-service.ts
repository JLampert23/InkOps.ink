...
export async function fetchLiveSanMarPricing(
  partId: string,
  companyId?: string
): Promise<{ price: number; priceTiers: Array<{ minQuantity: number; price: number }> } | null> {
  try {
    console.log(`[SanMar] Fetching live pricing for partId: ${partId}`);
    ...
    if (!effectiveCompanyId) {
      console.warn('[SanMar] No company ID available for pricing lookup');
      return null;
    }

    const sanmarApiUrl = `${import.meta.env.VITE_SUPABASE_URL}/functions/v1/sanmar-api`;
    const response = await fetch(
      `${sanmarApiUrl}?action=pricing&partId=${encodeURIComponent(partId)}&companyId=${encodeURIComponent(effectiveCompanyId)}`,
      {
        headers: {
          'Authorization': `Bearer ${import.meta.env.VITE_SUPABASE_ANON_KEY}`,
          'apikey': import.meta.env.VITE_SUPABASE_ANON_KEY,
    ...
    if (!response.ok) {
      console.error(`[SanMar] Pricing API returned ${response.status}`);
      return null;
    }

    const data = await response.json();

    if (data.success && data.data?.parts && data.data.parts.length > 0) {
      const partPricing = data.data.parts[0];
      if (partPricing.prices && partPricing.prices.length > 0) {
        const lowestTierPrice = partPricing.prices.reduce(
          (lowest: any, current: any) => {
            if (!lowest || current.minQuantity < lowest.minQuantity) return current;
            return lowest;
          },
          partPricing.prices[0]
        );

        console.log(`[SanMar] Live pricing found for ${partId}: $${lowestTierPrice.price}`);

        return {
          price: lowestTierPrice.price,
          priceTiers: partPricing.prices.map((p: any) => ({
    ...
    console.log(`[SanMar] No pricing data returned for ${partId}`);
    return null;
  } catch (error) {
    console.error('[SanMar] Error fetching live pricing:', error);
    return null;
  }
}
