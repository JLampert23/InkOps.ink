/*
  # Update Chipply Import to Store Artwork URLs
  
  1. Changes
    - Update process_chipply_import function to extract and store artwork URLs
    - Take the first artworkVariation's imageSrc as the artwork URL for each imprint
*/
...
BEGIN
  ...
  FOR i IN 0..(jsonb_array_length(v_processes) - 1) LOOP
    ...
    IF v_components IS NOT NULL AND jsonb_array_length(v_components) > 0 THEN
      v_component := v_components->0;
      v_artwork_variations := v_component->'artworkVariations';
      IF v_artwork_variations IS NOT NULL AND jsonb_array_length(v_artwork_variations) > 0 THEN
