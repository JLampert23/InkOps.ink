/*
  # Update Chipply Import to Lookup or Create Customers

  1. Changes
    - Search for existing customer by email first, then by company name
    - If no customer found, create a new customer record
    - Link the quote to the customer via customer_id
    
  2. Customer Matching Logic
    - First priority: Match by email (case-insensitive)
    - Second priority: Match by company_name (case-insensitive)
    - If no match found: Create new customer with Chipply data
*/
