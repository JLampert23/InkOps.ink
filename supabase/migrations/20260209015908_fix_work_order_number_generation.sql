/*
  # Fix Work Order Number Generation

  1. Changes
    - Updates work order number generation to match existing format (WO-YYYYMMDD-XXXXX)
    - Handles existing work order numbers correctly
*/
...
BEGIN
  IF NEW.status = 'approved' AND (OLD.status IS NULL OR OLD.status != 'approved') THEN
    ...
    -- Generate invoice number (INV-YYYYMMDD-XXXXX format)
    SELECT 'INV-' || to_char(now(), 'YYYYMMDD') || '-' || 
           LPAD(COALESCE(MAX(SUBSTRING(invoice_number FROM '\d+$'))::int, 0) + 1::text, 5, '0')
    INTO v_invoice_id
    ...
    IF v_invoice_id IS NULL THEN
      v_invoice_id := 'INV-' || to_char(now(), 'YYYYMMDD') || '-00001';
    END IF;
    ...
    -- Status stays as 'approved' - NOT changed to 'converted'
