/*
  # Fix Chipply Import - Process-Based Line Item Groups

  1. Overview
    - Each Chipply process becomes a separate line item group
    - Group label format: "Process 1: {processName}", "Process 2: {processName}", etc.
    - All garments within a process share the same group_label
    - Each process gets imprints with matching group_label
    - Create proofs for garment images linked to imprints
    - Remove decoration line items (decorations only exist as imprints)

  2. Changes
    - Assign unique group_label to each process: "Process {i+1}: {processName}"
    - Apply group_label to all line items within that process
    - Link imprints to process groups using matching group_label
    - Extract garment images and create proof records
    - Store all artwork variations in imprint artwork_images array
    - Populate individual size columns (qty_xs, qty_s, etc.) from Chipply size data

  3. Data Structure
    - Line items: Only garments with group_label set
    - Imprints: One per component, with group_label matching the process
    - Proofs: One per garment color with images, linked to line item and imprint
...
DECLARE
  ...
  v_group_label text;
  v_process_name text;
  v_garment_image_url text;
  v_garment_image_2_url text;
  v_garment_image_3_url text;
  v_size_elem jsonb;
  v_size_name text;
  v_size_qty integer;
  i integer;
  ...
  FOR i IN 0..(jsonb_array_length(v_processes) - 1) LOOP
    ...
    FOR j IN 0..(jsonb_array_length(v_products) - 1) LOOP
      ...
      FOR k IN 0..(jsonb_array_length(v_colors) - 1) LOOP
        ...
        -- Extract garment images
        v_garment_image_url := v_color->>'image1Url';
        v_garment_image_2_url := v_color->>'image2Url';
        v_garment_image_3_url := v_color->>'image3Url';
        ...
        v_size_data := jsonb_build_object(
          'qty_yxs', 0, 'qty_ys', 0, 'qty_ym', 0, 'qty_yl', 0, 'qty_yxl', 'qty_xxs', 0,
          'qty_xs', 0, 'qty_s', 0, 'qty_m', 0, 'qty_l', 0, 'qty_xl', 0,
          'qty_2xl', 0, 'qty_3xl', 0, 'qty_4xl', 0, 'qty_5xl', 0, 'qty_6xl', 0
        );
        ...
        LOOP
          ...
          CASE v_size_name
            ...
            WHEN 'XXS' THEN v_size_data := jsonb_set(v_size_data, '{qty_xxs}', to_jsonb(v_size_qty));
            ...
            WHEN '6XL' THEN v_size_data := jsonb_set(v_size_data, '{qty_6xl}', to_jsonb(v_size_qty));
          END CASE;
        END LOOP;
        ...
        INSERT INTO quote_line_items (
          ...
          qty_xxs, qty_xs, qty_s, qty_m, qty_l, qty_xl,
          ...
          (v_size_data->>'qty_yxl')::integer, (v_size_data->>'qty_xxs')::integer,
          ...
          (v_size_data->>'qty_5xl')::integer, (v_size_data->>'qty_6xl')::integer,
          COALESCE((v_color->>'productPrice')::numeric, 0),
          COALESCE((v_color->>'productPrice')::numeric, 0) * v_total_qty,
          v_line_notes,
          v_garment_image_url,
          v_garment_image_2_url,
          v_garment_image_3_url,
          now()
        )
        ...
        IF v_garment_image_url IS NOT NULL THEN
