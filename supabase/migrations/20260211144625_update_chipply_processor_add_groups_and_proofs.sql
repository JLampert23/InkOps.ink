/*
  # Update Chipply Processor - Add Line Item Groups and Garment Proofs

  1. Changes
    - Each decoration process becomes its own line item group
    - All garments and decorations for a process share the same group_label
    - Creates garment proof records for each imported garment
    - Links proofs to the quote_imprints (decoration processes)
    - Uses image URLs from the Chipply productColors data

  2. Notes
    - group_label format: "Process {N}: {processName}"
    - Proofs are created with garment images and linked to imprint_id
    - Each color variation gets its own proof record
*/
      ...
      IF v_components_count > 0 THEN
        FOR m IN 0..(v_components_count - 1) LOOP
          ...
          INSERT INTO quote_line_items (
            ...
            decoration_location, notes, group_label, created_at
          ) VALUES (
