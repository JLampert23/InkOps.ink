/*
  # Fix Standalone Chipply Processor Quote Number Generation

  ## Problem
  The standalone `process_chipply_import(uuid)` function uses `company_settings.quote_start_number`
  which can become stale and out-of-sync with actual quote numbers, causing duplicate key violations.

  ## Solution
  Use the same MAX-based approach as the trigger function to dynamically calculate the next quote number.
  
  ## Changes
  1. Replace company_settings-based numbering with MAX-based scan of existing quotes
  2. This ensures no duplicate quote numbers even if company_settings is out of sync
*/
...
BEGIN
  ...
  -- FIXED: Use MAX-based quote number generation instead of company_settings
  ...
  FOR i IN 0..(jsonb_array_length(v_processes) - 1) LOOP
    ...
    FOR j IN 0..(jsonb_array_length(v_products) - 1) LOOP
      ...
      FOR k IN 0..(jsonb_array_length(v_colors) - 1) LOOP
        ...
        -- FIXED: Use styleName for item_number (was styleNumber)
