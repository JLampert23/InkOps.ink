/*
  # Fix JSONB Operator Chaining in Chipply Import
  
  1. Changes
    - Store intermediate JSONB values in variables
    - Avoid chaining operators that cause type confusion
*/
  ...
  FOR i IN 0..(jsonb_array_length(v_processes) - 1) LOOP
    ...
    IF v_components IS NOT NULL AND jsonb_typeof(v_components) = 'array' AND jsonb_array_length(v_components) > 0 THEN
      ...
      IF v_component IS NOT NULL AND jsonb_typeof(v_component) = 'object' THEN
        ...
        IF v_artwork_variations IS NOT NULL AND 
           jsonb_typeof(v_artwork_variations) = 'array' AND 
           jsonb_array_length(v_artwork_variations) > 0 THEN
