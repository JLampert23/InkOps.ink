/*
  # Fix work order initial status in process_quote_approval

  The trigger was setting the work order status to a production workflow column
  name (e.g. 'Pending Scheduling') which violates the work_orders_status_check
  constraint that only allows: draft, in_progress, completed, cancelled, on_hold.

  ## Fix
  - Work order status now defaults to 'draft' instead of a workflow column name
*/
...
BEGIN
  IF NEW.status = 'approved' AND (OLD.status IS NULL OR OLD.status != 'approved') THEN
    ...
    -- 4a. POPULATE WORK ORDER LINE ITEMS
    FOR v_line_item IN
      SELECT
        id,
        line_number,
    ...
    -- 5a. POPULATE INVOICE LINE ITEMS
    FOR v_line_item IN
      SELECT
        id,
        line_number,
