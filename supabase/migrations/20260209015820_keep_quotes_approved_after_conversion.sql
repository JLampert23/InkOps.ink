/*
  # Keep Quotes in Approved Status After Conversion

  1. Changes
    - Updates `process_quote_approval()` function to NOT change status to 'converted'
    - Quotes will stay in 'approved' status even after work orders and invoices are created
    - All conversion actions (work order, invoice, staging, etc.) still happen
    - User can now see and use the "Edit Quote" button on approved quotes

  2. Purpose
    - Allow editing of approved quotes via "Edit Quote" button
    - Maintain visibility of approved quotes in the quotes list
    - Keep conversion metadata (converted_at, production_job_id) for tracking
*/
...
BEGIN
  IF NEW.status = 'approved' AND (OLD.status IS NULL OR OLD.status != 'approved') THEN
    ...
    SELECT COALESCE('INV-' || LPAD((
      SELECT COALESCE(MAX(CAST(SUBSTRING(invoice_number FROM 'INV-(.*)') AS INTEGER)), 0) + 1
      FROM printavo_invoices WHERE company_id = NEW.company_id
    )::text, 7, '0'), 'INV-0000001')
    INTO v_invoice_id;
    ...
    VALUES (v_invoice_id, NEW.company_id, v_invoice_id, NEW.customer_email, NEW.customer_name,
      ...
      0, NEW.total, 'Open', 'unpaid', CURRENT_DATE,
    ...
    -- Set converted metadata BUT keep status as 'approved'
    NEW.converted_at := now();
    NEW.production_job_id := v_work_order_id;
    -- REMOVED: NEW.status := 'converted';  <- This line is intentionally removed
