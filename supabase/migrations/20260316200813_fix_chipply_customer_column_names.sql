/*
  # Fix Chipply Customer Creation Column Names

  1. Changes
    - Update process_chipply_import function to use correct customer table column names
    - Change billing_address_1 to billing_address_line1
    - Change billing_address_2 to billing_address_line2
    
  2. Issue
    - The customers table uses billing_address_line1/line2 naming convention
    - The Chipply import function was using billing_address_1/2 (incorrect)
    - This caused "column does not exist" errors when creating customers from Chipply imports
*/
...
BEGIN
  ...
  -- First, try to find customer by email (most reliable match)
  ...
  -- If not found by email, try to find by company name / customer name
  IF v_customer_id IS NULL AND v_customer_name IS NOT NULL AND v_customer_name != '' THEN
    SELECT id INTO v_customer_id
    FROM customers
    WHERE company_id = v_company_id
      AND LOWER(company_name) = LOWER(v_customer_name)
    LIMIT 1;
  END IF;
  ...
  INSERT INTO quotes (
    company_id, quote_number, customer_id, customer_name, customer_email, customer_phone,
    ...
    v_company_id, v_quote_number, v_customer_id, v_customer_name, v_customer_email, v_customer_phone,
