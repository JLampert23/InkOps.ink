/*
  # Complete Rewrite of Chipply Import to Avoid Operator Chaining
  
  1. Changes
    - Completely avoid chaining JSONB operators
    - Use explicit intermediate variables for every operation
    - Cast all values explicitly
*/
...
BEGIN
  ...
  -- Extract customer data using explicit casts
  SELECT 
    v_account_summary->>'customerName',
    (v_account_summary->>'dueDate')::date
  INTO v_customer_name, v_due_date;
