/*
  # Fix Chipply Import JSONB Operator Type Error

  1. Problem
    - The function uses `v_color->>'colorName'` but v_color is jsonb type
    - Should use ->> operator correctly with jsonb variables
    - Error: "operator does not exist: text ->> unknown"

  2. Solution
    - All jsonb variables already declared correctly
    - Issue is concatenation order causing type confusion
    - Fix by ensuring proper operator usage
*/
  ...
  FOR i IN 0..(jsonb_array_length(v_processes) - 1) LOOP
    ...
    FOR j IN 0..(jsonb_array_length(v_products) - 1) LOOP
      ...
      FOR k IN 0..(jsonb_array_length(v_colors) - 1) LOOP
        ...
        -- Build description and notes as text variables first
    ...
    IF v_components IS NOT NULL AND jsonb_typeof(v_components) = 'array' AND jsonb_array_length(v_components) > 0 THEN
      FOR m IN 0..(jsonb_array_length(v_components) - 1) LOOP
        ...
        v_line_notes := 'Location: ' || COALESCE(v_component->>'artworkLocationName', '') ||
