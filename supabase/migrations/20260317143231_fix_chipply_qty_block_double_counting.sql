/*
  # Fix Chipply Qty Block Double Counting

  1. Problem
    - Chipply import sets both individual size columns AND the quantity field
    - Quote View displays size totals + quantity field, causing double counting
    - Quote Builder displays 0 in Qty block (correct behavior)
    - Result: Chipply imports show incorrect totals in Quote View

  2. Solution
    - Update process_chipply_import function to NOT populate quantity field for garment items
    - Leave quantity field as 0 for items with size breakdowns
    - Quote View will now match Quote Builder behavior (Qty = 0, Items = sum of sizes)

  3. Changes
    - Line 408: Set quantity to 0 instead of v_total_qty
    - This ensures the Qty block displays 0 while Items shows the correct size total
*/
