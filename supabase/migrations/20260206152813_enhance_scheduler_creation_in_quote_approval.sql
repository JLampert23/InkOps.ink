/*
  # Enhance Scheduler Task Creation in Quote Approval

  1. Updates
    - Modify `process_quote_approval()` to include enhanced scheduler metadata
    - Add work_order_id link
    - Include colors, department, estimated runtime
    - Set proper scheduler_column (will auto-default to "Unscheduled")
    - Enable auto-assignment based on rules

  2. Purpose
    - Create fully-populated scheduler tasks
    - Enable immediate production tracking
    - Support auto-assignment workflows
*/

-- Drop and recreate the approval automation function with enhanced scheduler creation
...
BEGIN
  ...
  IF NEW.status = 'approved' AND (OLD.status IS NULL OR OLD.status != 'approved') THEN
    ...
    -- 7. PUSH IMPRINTS TO SCHEDULER WITH ENHANCED METADATA
    FOR v_imprint IN
      SELECT
        qi.id as imprint_id,
        qi.line_item_id,
...
COMMENT ON FUNCTION process_quote_approval() IS 'Comprehensive automation: locks quote, creates work order with line items, invoice with line items, stages POs, pushes to scheduler with enhanced metadata and auto-assignment';
