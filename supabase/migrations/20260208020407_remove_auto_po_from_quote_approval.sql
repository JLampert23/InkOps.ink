/*
  # Remove Auto-PO Creation from Quote Approval

  1. Changes
    - Removes the automatic Purchase Order creation from `process_quote_approval()`
    - Keeps all other approval logic intact:
      - Work Order creation
      - Invoice creation
      - Billing queue entry
      - Scheduler entries
      - Garment requirements staging (so items appear in Garment Purchase Report)
      - Quote activity logging

  2. New Function
    - `add_garment_items_to_po()` - Adds garment line items to an existing or new PO
      - Accepts vendor_id and an array of garment item details
      - Checks for same-day PO for the vendor; reuses it or creates new
      - Returns the PO id and PO number

  3. Important Notes
    - Users will now manually add items to POs from the Garment Purchase Report
    - The garment_requirements_staging table continues to be populated on approval
    - No other modules are affected
*/

-- Recreate the trigger function WITHOUT auto-PO creation
...
BEGIN
  IF NEW.status = 'approved' AND (OLD.status IS NULL OR OLD.status != 'approved') THEN
    ...
    -- Stage Garment Requirements (for Garment Purchase Report)
    FOR v_line_item IN
    ...
    -- NO AUTO-PO CREATION -- Users add items to POs manually from Garment Purchase Report
...
-- Function: Add garment items to a PO (same-day merge or create new)
CREATE OR REPLACE FUNCTION add_garment_items_to_po(
  p_company_id uuid,
  p_vendor_id uuid,
  p_items jsonb
) RETURNS jsonb
...
BEGIN
  ...
  IF v_po_id IS NULL THEN
    ...
    INSERT INTO purchase_orders (
      company_id, po_number, vendor_id, status, internal_notes, created_by
    ) VALUES (
      p_company_id, v_po_number, p_vendor_id, 'draft',
      'Created from Garment Purchase Report',
      auth.uid()
    ) RETURNING id INTO v_po_id;

    INSERT INTO purchase_order_activity_log (
      company_id, po_id, action, performed_by, performed_by_name, notes
    ) VALUES (
      p_company_id, v_po_id, 'created', auth.uid(), 'User', 'PO created from Garment Purchase Report'
    );
  END IF;

  SELECT COALESCE(MAX(line_number), 0) INTO v_line_number
  FROM purchase_order_line_items WHERE po_id = v_po_id;
  ...
  LOOP
    ...
    INSERT INTO purchase_order_line_items (
      ...
      p_company_id, v_po_id, v_line_number,
      v_item->>'style_number',
      v_item->>'product_name',
      v_item->>'color',
      v_item->>'size',
  ...
  SET subtotal = (SELECT COALESCE(SUM(extended_cost), 0) FROM purchase_order_line_items WHERE po_id = v_po_id),
      total_cost = (SELECT COALESCE(SUM(extended_cost), 0) FROM purchase_order_line_items WHERE po_id = v_po_id)
  WHERE id = v_po_id;

  INSERT INTO purchase_order_activity_log (
    company_id, po_id, action, performed_by, performed_by_name, notes
  ) VALUES (
    p_company_id, v_po_id, 'items_added_from_report', auth.uid(), 'User',
    'Added ' || jsonb_array_length(p_items) || ' item(s) from Garment Purchase Report'
  );
  ...
  RETURN jsonb_build_object(
    ...
    'is_new_po', v_is_new_po,
    'items_added', jsonb_array_length(p_items)
  );
END;
$$;

GRANT EXECUTE ON FUNCTION add_garment_items_to_po(uuid, uuid, jsonb) TO authenticated;