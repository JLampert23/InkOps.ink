/*
  # Fix Chipply Processor Line Item Columns

  1. Changes
    - Update process_chipply_import() to use correct quote_line_items columns:
      - Use `description` instead of `name`
      - Use `brand` for style information
      - Use `supplier_name` instead of `vendor` and `supplier_vendor`
      - Use `wholesale_price` instead of `cost`
      - Use `unit_price` instead of `price`
      - Use `regular_sizes` instead of `sizes`
      - Use `supplier_partid` instead of `supplier_part_id`
  ...
  LOOP
    ...
    LOOP
      ...
      LOOP
        ...
        -- Create Line Item for this color variant (using correct column names)
        ...
        LOOP
          -- Store component data in notes field
