/*
  # Fix Chipply Import Trigger - Remove Non-existent quote_id Column Reference

  ## Problem
  The trigger function `process_chipply_import()` was trying to update a `quote_id` column 
  on `chipply_import_logs` table that doesn't exist, causing all imports to fail with 
  "duplicate key value violates unique constraint" error (the actual error was masked).

  ## Changes
  1. Remove the `quote_id = v_quote_id` from the UPDATE statement
  2. Keep all other improvements from the previous migration (styleName for item_number)
  
  ## Note
  The relationship is already tracked via `quotes.chipply_import_log_id` (reverse direction)
*/
...
BEGIN
  IF NEW.status = 'pending' THEN
    ...
    FOR i IN 0..jsonb_array_length(v_work_order_data->'processes') - 1 LOOP
      ...
      FOR j IN 0..jsonb_array_length(v_process->'products') - 1 LOOP
        ...
        FOR k IN 0..jsonb_array_length(v_product->'productColors') - 1 LOOP
          ...
          -- Create line item - Use styleName for item_number
    ...
    -- FIXED: Remove quote_id reference - column doesn't exist on chipply_import_logs
    UPDATE chipply_import_logs 
    SET status = 'processed',
        updated_at = now()
    WHERE id = NEW.id;
