/*
  # Fix Work Order and Invoice numbering to match Quote number

  1. Changes
    - Updates `process_quote_approval()` trigger so the work order number
      and invoice ID/number use the quote number directly (e.g. Q2600001)
      instead of WO-<digits> / INV-<digits>.
    - This keeps quote, work order, and invoice all under one unified number.

  2. Important Notes
    - Only the numbering assignment lines change; all other logic is untouched.
*/
...
BEGIN
  IF NEW.status = 'approved' AND (OLD.status IS NULL OR OLD.status != 'approved') THEN
    ...
    v_work_order_number := NEW.quote_number;
    v_invoice_id := NEW.quote_number;
