/*
  # Fix Chipply Processor Company Settings Query

  1. Changes
    - Update process_chipply_import() to use `id` instead of `company_id` 
      when querying company_settings table
    - The company_settings table uses `id` as the primary key, not `company_id`
...
BEGIN
  ...
  -- FIX: Use `id` instead of `company_id` for company_settings table
  SELECT 
