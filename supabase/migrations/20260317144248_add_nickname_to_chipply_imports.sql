/*
  # Add Nickname to Chipply Imports

  1. Purpose
    - Populate the nickname field for Chipply-imported quotes
    - Format: "[Store Name] - Batch [Batch ID] - [Batch Date]"
    - Makes it easier to identify and search Chipply quotes in Quote Builder

  2. Changes
    - Add v_nickname variable declaration
    - Extract batch date from Chipply payload
    - Build nickname string with store name, batch ID, and batch date
    - Add nickname to INSERT INTO quotes statement

  3. Fallbacks
    - If store name missing: use "Chipply Import"
    - If batch ID missing: use "N/A"
    - If batch date missing: use due date or empty string
*/
...
BEGIN
  ...
  -- Build nickname from store name, batch ID, and batch date
  v_nickname := '';
  ...
  IF v_batch_date IS NOT NULL THEN
    v_nickname := v_nickname || ' - ' || TO_CHAR(v_batch_date, 'YYYY-MM-DD');
  ELSIF v_due_date IS NOT NULL THEN
    v_nickname := v_nickname || ' - ' || TO_CHAR(v_due_date, 'YYYY-MM-DD');
  END IF;
  ...
  -- Create the Quote with nickname
  INSERT INTO quotes (
    company_id,
    quote_number,
    customer_id,
  ...
  RAISE NOTICE 'Created quote % (nickname: %) for customer %', v_quote_number, v_nickname, v_customer_company;
  ...
  FOR i IN 0..(jsonb_array_length(v_processes) - 1) LOOP
    ...
    FOR j IN 0..(jsonb_array_length(v_products) - 1) LOOP
      ...
      FOR k IN 0..(jsonb_array_length(v_colors) - 1) LOOP
        ...
        -- Extract garment images (map to correct column names)
        ...
        -- IMPORTANT: Set quantity to 0 for garment items (size breakdowns handle the quantity)
        INSERT INTO quote_line_items (
          ...
          v_color->>'colorName',
          0,  -- Set to 0 instead of v_total_qty to prevent double counting
