/*
  # Fix Delivery Date Column Bug in Quote Approval Trigger

  1. Problem
    - The `process_quote_approval()` trigger references `NEW.delivery_date`
    - But the quotes table doesn't have a `delivery_date` column
    - It should use `customer_due_date` instead
    
  2. Changes
    - Replace all `NEW.delivery_date` references with `NEW.customer_due_date`
    - Fixes lines 61, 114, and 538 in the trigger function
    
  3. Impact
    - Quote approvals will now work correctly
    - Work orders and scheduler entries will get correct due dates
*/

-- Replace the quote approval function with fixed column references
  ...
  IF NEW.status = 'approved' AND (OLD.status IS NULL OR OLD.status != 'approved') THEN
    ...
    IF v_is_reapproval THEN
      ...
      SET
        ...
        due_date = NEW.customer_due_date,
        notes = NEW.notes,
        updated_at = now()
