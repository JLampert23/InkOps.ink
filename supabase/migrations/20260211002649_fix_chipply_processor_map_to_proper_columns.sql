/*
  # Fix Chipply Processor - Map to Proper Columns

  1. Changes
    - Use `item_number` for product style/SKU
    - Use `color` for color name
    - Parse sizes and populate individual size columns (qty_xs, qty_s, qty_m, etc.)
    - Set proper `line_number` for ordering
    - Use `line_type` to distinguish garments from decorations
*/
...
BEGIN
  ...
  FOR i IN 0..(v_processes_length - 1) LOOP
    ...
    INSERT INTO quote_imprints (
      ...
      COALESCE(v_process->>'processName', 'Process ' || i::text),
