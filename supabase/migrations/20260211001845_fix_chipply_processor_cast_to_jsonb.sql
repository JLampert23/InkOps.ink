/*
  # Fix Chipply Processor - Cast Record Fields to JSONB

  1. Changes
    - Cast record fields back to jsonb type when accessing nested data
*/
...
BEGIN
  ...
  FOR v_process_elem IN 
    SELECT value FROM jsonb_array_elements(v_work_order_data->'processes')
  LOOP
    v_process := v_process_elem.value::jsonb;
    ...
    FOR v_product_elem IN 
      SELECT value FROM jsonb_array_elements(v_process->'products')
    LOOP
      v_product := v_product_elem.value::jsonb;
      
      -- Process each color variant (productColors array)
      FOR v_color_elem IN 
        SELECT value FROM jsonb_array_elements(v_product->'productColors')
      LOOP
        v_color := v_color_elem.value::jsonb;
    ...
    FOR v_component_elem IN 
      SELECT value FROM jsonb_array_elements(v_process->'components')
    LOOP
      v_component := v_component_elem.value::jsonb;
