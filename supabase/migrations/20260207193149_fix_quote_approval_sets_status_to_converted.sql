/*
  # Fix quote approval trigger to set status to 'converted'

  1. Changes
    - Updates the process_quote_approval trigger function to set quote status = 'converted'
      after creating work order, invoice, and all related records
    - This ensures approved quotes move out of the Quotes tab and their work orders
      appear in the Work Orders tab

  2. Notes
    - Previously the trigger set converted_at but left the status as 'approved'
    - Now the status is set to 'converted' so filtering works correctly
*/
...
BEGIN
  IF NEW.status = 'approved' AND (OLD.status IS NULL OR OLD.status != 'approved') THEN
    ...
    SELECT COALESCE(MAX(SUBSTRING(work_order_number FROM '\d+$')::int), 0) + 1 INTO v_next_wo_num
