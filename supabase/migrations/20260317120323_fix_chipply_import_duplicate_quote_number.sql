/*
  # Fix Chipply Import Duplicate Quote Number Error

  1. Problem
    - The process_chipply_import function increments quote_number immediately
    - If the function fails partway through, the quote number is consumed but the import isn't marked as processed
    - Subsequent imports try to reuse the same quote number, causing duplicate key violations

  2. Solution
    - Add idempotency check to prevent reprocessing already-processed imports
    - Use row-level locking (SELECT FOR UPDATE) to prevent race conditions
    - Only increment quote number AFTER successful quote creation
    - Add Chipply sale order ID tracking to prevent duplicate imports
    - Wrap everything in proper error handling

  3. Changes
    - Add chipply_sale_order_id column to quotes table for deduplication
    - Rewrite process_chipply_import function with transaction safety
    - Add status check to prevent reprocessing
*/

-- Add chipply_sale_order_id to quotes table for tracking
DO $$ 
BEGIN
  IF NOT EXISTS (
    SELECT 1 FROM information_schema.columns 
    WHERE table_name = 'quotes' AND column_name = 'chipply_sale_order_id'
  ) THEN
    ALTER TABLE quotes ADD COLUMN chipply_sale_order_id text;
    CREATE INDEX IF NOT EXISTS idx_quotes_chipply_sale_order_id ON quotes(chipply_sale_order_id);
  END IF;
END $$;

-- Drop and recreate the function with proper transaction handling
...
BEGIN
  ...
  IF v_log_record.status IN ('processed', 'processing') THEN
    RAISE NOTICE 'Import log % already in status: %, skipping', log_id, v_log_record.status;
    RETURN;
  END IF;
