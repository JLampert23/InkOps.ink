/*
  # Fix Chipply Artwork Extraction
  
  1. Changes
    - Fix JSONB operator usage in process_chipply_import function
    - Properly handle NULL checks for components and artworkVariations
*/
...
BEGIN
  ...
  FOR i IN 0..(jsonb_array_length(v_processes) - 1) LOOP
    ...
    -- Extract artwork URL from first component's first variation (if available)
    v_artwork_url := NULL;
    ...
    IF v_components IS NOT NULL AND jsonb_typeof(v_components) = 'array' AND jsonb_array_length(v_components) > 0 THEN
      ...
      IF v_component IS NOT NULL AND jsonb_typeof(v_component) = 'object' THEN
        -- Check if artworkVariations exists and is an array
        IF (v_component->'artworkVariations') IS NOT NULL AND 
           jsonb_typeof(v_component->'artworkVariations') = 'array' AND 
           jsonb_array_length(v_component->'artworkVariations') > 0 THEN
          v_artwork_url := (v_component->'artworkVariations')->0->>'imageSrc';
        END IF;
      END IF;
    END IF;
