/*
  # Fix Chipply Processor to Use Correct Quotes Columns

  1. Changes
    - Update process_chipply_import() to use correct quotes table columns:
      - Use `customer_due_date` instead of `due_date`
      - Use `subtotal` instead of `total_cost`
...
BEGIN
  ...
  -- FIX: Use correct column names for quotes table
  INSERT INTO quotes (
    company_id,
    quote_number,
    customer_name,
