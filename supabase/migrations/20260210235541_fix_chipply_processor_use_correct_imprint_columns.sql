/*
  # Fix Chipply Processor to Use Correct Imprint Columns

  1. Changes
    - Update process_chipply_import() to use existing quote_imprints columns:
      - Use `type_of_work` instead of `imprint_type`
      - Use `details` instead of `imprint_description`
      - Remove `due_date` since it doesn't exist in quote_imprints
...
BEGIN
  ...
  LOOP
    -- Create Imprint for this process (using correct column names)
