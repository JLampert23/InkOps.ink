/*
  # Fix Chipply Import - Set price_locked to Preserve Unit Price

  ## Problem
  When Chipply imports line items with `unit_price` set from `productPrice`,
  the `propagate_group_unit_price` trigger fires immediately after INSERT and
  recalculates `unit_price` using `imprint_prices + wholesale_price * markup`.
  Since Chipply items have no `wholesale_price` and imprint prices start at 0,
  this overwrites the correct Chipply price with 0.

  ## Fix
  Set `price_locked = true` on all Chipply-imported line items so the
  `propagate_group_unit_price` function skips recalculation for that group.
  The `garment_unit_price` already stores the Chipply selling price correctly.

  ## Changes
  1. Update trigger function `process_chipply_import()` to insert with `price_locked = true`
  2. Update standalone function `process_chipply_import(uuid)` to insert with `price_locked = true`
  3. Backfill existing Chipply quotes to set `price_locked = true` and restore `unit_price` from `garment_unit_price`
*/
...
-- Backfill existing Chipply quotes: restore unit_price from garment_unit_price
-- and set price_locked = true so pricing trigger won't zero them out
UPDATE quote_line_items qli
SET 
  price_locked = true,
  unit_price = COALESCE(garment_unit_price, unit_price),
  total_price = COALESCE(garment_unit_price, 0) * (
...
WHERE EXISTS (
  ...
  AND q.chipply_sale_order_id IS NOT NULL
)
AND qli.garment_unit_price IS NOT NULL
AND qli.garment_unit_price > 0;

-- Update quote totals for affected Chipply quotes
UPDATE quotes q
SET 
  subtotal = (SELECT COALESCE(SUM(total_price), 0) FROM quote_line_items WHERE quote_id = q.id),
  total = (SELECT COALESCE(SUM(total_price), 0) FROM quote_line_items WHERE quote_id = q.id)
WHERE chipply_sale_order_id IS NOT NULL;
