/*
  # Fix PO Creation to Use Company Distributor Settings

  1. Changes
    - Updates `process_quote_approval()` to determine the correct distributor from `company_settings`
    - Checks `ssactivewear_enabled` and `sanmar_enabled` flags to resolve the vendor
    - If a line item has an explicit `supplier_name`, that is respected
    - If not, the company's enabled distributor is used
    - Removes brand-name guessing logic that incorrectly assumed Gildan = SanMar
    - Also fixes garment_requirements_staging to use the correct resolved supplier

  2. Bug Fix
    - Previously, brand names were used to guess the distributor (e.g., Gildan -> SanMar)
    - This was incorrect because distributors carry overlapping brands
    - The company_settings table already tracks which distributors a company uses
    - Now the trigger reads those settings to determine the correct vendor
*/
...
BEGIN
  IF NEW.status = 'approved' AND (OLD.status IS NULL OR OLD.status != 'approved') THEN
    ...
    -- Determine the company's default distributor from settings
    ...
    -- Stage Garment Requirements using company distributor settings
    FOR v_line_item IN
    ...
    FOR v_supplier_group IN
      SELECT
        COALESCE(NULLIF(qli.supplier_name, ''), v_default_distributor_name) as resolved_name,
        CASE
          WHEN COALESCE(NULLIF(qli.supplier_name, ''), '') != '' THEN
            CASE
              WHEN LOWER(qli.supplier_name) LIKE '%sanmar%' THEN 'sanmar'
              WHEN LOWER(qli.supplier_name) LIKE '%ssactivewear%' OR LOWER(qli.supplier_name) LIKE '%s&s%' THEN 'ssactivewear'
              ELSE 'other'
            END
          ELSE v_default_distributor_type
    ...
    LOOP
      ...
      FOR v_line_item IN
        SELECT item_number, brand, color, regular_sizes, description,
          ...
          COALESCE(NULLIF(supplier_name, ''), v_default_distributor_name) as computed_supplier
      ...
      LOOP
        IF v_line_item.computed_supplier = v_supplier_group.resolved_name THEN
          IF v_line_item.regular_sizes IS NOT NULL AND jsonb_typeof(v_line_item.regular_sizes) = 'object'
             AND (SELECT COUNT(*) FROM jsonb_each_text(v_line_item.regular_sizes) WHERE value::text ~ '^\d+$' AND value::text::integer > 0) > 0 THEN
            FOR v_size_key, v_size_qty IN
              SELECT key, value::text::integer
              FROM jsonb_each_text(v_line_item.regular_sizes)
              WHERE value::text ~ '^\d+$' AND value::text::integer > 0
            LOOP
      ...
      WHERE quote_id = NEW.id AND is_po_created = false
        AND COALESCE(NULLIF(supplier_name, ''), v_default_distributor_name) = v_supplier_group.resolved_name;
