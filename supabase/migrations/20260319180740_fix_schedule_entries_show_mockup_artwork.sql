/*
  # Fix Schedule Entries to Show Mockup Artwork

  1. Changes
    - Update the quote approval trigger to populate artwork_thumb_url from quote_imprints.mockups
    - Backfill existing schedule entries with mockup URLs
    - Add helper function to extract first mockup URL from JSONB array

  2. Purpose
    - Display artwork/mockup images in the scheduling tab
    - Use the first mockup from the mockups array as the thumbnail
*/

-- Helper function to extract the first mockup URL from the mockups JSONB array
...
BEGIN
  ...
  RETURN mockups_array->0->>'file_url';
END;
$$ LANGUAGE plpgsql IMMUTABLE;

-- Update the process_quote_approval function to use mockups instead of artwork_url
...
BEGIN
  ...
  IF NEW.status = 'approved' AND (OLD.status IS NULL OR OLD.status != 'approved') THEN
    ...
    -- 7. PUSH IMPRINTS TO SCHEDULER WITH MOCKUP ARTWORK
    FOR v_imprint IN
      SELECT
        qi.id as imprint_id,
        qi.line_item_id,
    ...
    LOOP
      -- Determine department based on type of work
      v_department := CASE
      ...
      INSERT INTO production_schedule_entries (
        ...
        NEW.id,
        v_work_order_id,
        v_imprint.line_item_id,
        v_imprint.imprint_id,
        v_imprint.type_of_work,
        v_imprint.imprint_number,
        get_first_mockup_url(v_imprint.mockups),
        COALESCE(NEW.production_due_date, NEW.customer_due_date, CURRENT_DATE + INTERVAL '7 days'),
        COALESCE(v_imprint.quantity, 0),
        '{}'::jsonb,
        0,
        COALESCE(v_imprint.customer_name, NEW.customer_name),
        NEW.quote_number,
        v_imprint.thread_ink_color,
        COALESCE(v_imprint.estimated_runtime_minutes, 0),
        v_department,
        NEW.notes
      )
...
COMMENT ON FUNCTION process_quote_approval() IS 'Comprehensive automation: locks quote, creates work order with line items, invoice with line items, stages POs, pushes to scheduler with mockup artwork and auto-assignment';
...
COMMENT ON FUNCTION get_first_mockup_url(jsonb) IS 'Extracts the first mockup file_url from a mockups JSONB array';
