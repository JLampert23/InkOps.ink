/*
  # Fix Work Orders Column Mismatch in Quote Approval Trigger

  1. Problem
    - Trigger references `customer_company` and `due_date` columns that don't exist in work_orders
    - work_orders has: `customer_due_date`, `production_due_date` (no `due_date` or `customer_company`)
    
  2. Changes
    - Remove `customer_company` reference (work_orders doesn't track company name)
    - Replace `due_date` with `customer_due_date`
    - Also fix INSERT INTO work_orders to remove non-existent columns
    
  3. Impact
    - Quote approvals will now complete successfully
*/
  ...
  IF NEW.status = 'approved' AND (OLD.status IS NULL OR OLD.status != 'approved') THEN
    ...
    IF v_is_reapproval THEN
      ...
      SET
        ...
        customer_due_date = COALESCE(NEW.customer_due_date, CURRENT_DATE + INTERVAL '7 days')::date,
        production_due_date = COALESCE(NEW.production_due_date, CURRENT_DATE + INTERVAL '5 days')::date,
        notes = NEW.notes,
        updated_at = now()
    ...
    ELSE
      ...
      INSERT INTO work_orders (
        ...
        NEW.customer_name,
        'pending_scheduling',
        COALESCE(NEW.customer_due_date, CURRENT_DATE + INTERVAL '7 days')::date,
        COALESCE(NEW.production_due_date, CURRENT_DATE + INTERVAL '5 days')::date,
        NEW.notes,
        NEW.company_id
      )
    ...
    INSERT INTO production_schedule_entries (
      ...
      NEW.id,
      NEW.customer_name,
      COALESCE(NEW.customer_due_date, CURRENT_DATE + INTERVAL '7 days'),
      'pending',
