/*
  # Fix Chipply Import - JSONB Build Object Syntax Error

  1. Problem
    - jsonb_build_object has odd number of arguments (missing value for qty_yxl)
    - Line: 'qty_yxl', 'qty_xxs', 0 should be 'qty_yxl', 0, 'qty_xxs', 0

  2. Solution
    - Fix the jsonb_build_object to have proper key-value pairs
*/
  ...
  FOR i IN 0..(jsonb_array_length(v_processes) - 1) LOOP
    ...
    FOR j IN 0..(jsonb_array_length(v_products) - 1) LOOP
      ...
      FOR k IN 0..(jsonb_array_length(v_colors) - 1) LOOP
        ...
        -- Initialize all size columns to 0 (FIXED: proper key-value pairs)
        v_size_data := jsonb_build_object(
          'qty_yxs', 0, 'qty_ys', 0, 'qty_ym', 0, 'qty_yl', 0, 'qty_yxl', 0, 'qty_xxs', 0,
