/*
  # Fix Chipply Image Column Alignment

  1. Problem
    - Chipply import saves images to: garment_image_url, garment_back_image_url, garment_side_image_url
    - QuoteBuilder/QuoteDetail read from: garment_front_image_url, garment_rear_image_url, garment_side_image_url
    - This mismatch causes Chipply quote images not to display

  2. Solution
    - Update process_chipply_import() to save images to BOTH sets of columns
    - Use garment_front_image_url as primary (matching UI expectations)
    - Also populate garment_image_url for backward compatibility
*/
...
BEGIN
  ...
  INSERT INTO quotes (
    ...
    log_id,
    v_sale_order_id,
    COALESCE(v_account_summary->>'parentStoreName', 'Chipply Order') || ' – Batch ' || COALESCE(v_account_summary->>'batchId', 'N/A'),
    now()
  )
  ...
  FOR i IN 0..(jsonb_array_length(v_processes) - 1) LOOP
    ...
    FOR j IN 0..(jsonb_array_length(v_products) - 1) LOOP
      ...
      FOR k IN 0..(jsonb_array_length(v_colors) - 1) LOOP
        ...
        -- Extract garment images (use UI-compatible column names)
        v_garment_front_image_url := v_color->>'image1Url';
        v_garment_rear_image_url := v_color->>'image2Url';
        ...
        -- Create Line Item with BOTH sets of image columns for compatibility
