/*
  # Fix Automated Reports with 2-Hour Window and Catch-Up Logic
  
  1. Changes to process_automated_reports Function
    - Expand execution window from 15 minutes to 2 hours
    - Add catch-up logic: if report hasn't been sent today and scheduled time has passed, send it
    - Fix timezone handling to properly calculate "today" in user's local timezone
    - Add comprehensive logging to execution log table
    - Track detailed decision logic for debugging
  
  2. Key Improvements
    - Reports will send anytime within 2 hours of scheduled time
    - If a report is checked after the 2-hour window but still the same day, it will catch up
    - All executions are logged with detailed reasoning
    - Timezone-aware date comparisons prevent duplicate sends
  
  3. Logic
    - Convert scheduled time to UTC for comparison
    - Check if current time is past scheduled time
    - Check if report was already sent "today" in the report's timezone
    - Send if: (1) within 2-hour window OR (2) past scheduled time but not sent today yet
*/
