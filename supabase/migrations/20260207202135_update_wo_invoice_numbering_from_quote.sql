/*
  # Update WO and Invoice numbering to carry over Quote numeric ID

  1. Changes
    - Modifies the process_quote_approval trigger function so that
      the Work Order number and Invoice number reuse the same numeric
      portion as the originating Quote number, with only the prefix changing.
    - Example: QTE-1045 -> WO-1045 -> INV-1045

  2. Number Extraction
    - Extracts all digits from the quote_number field
    - Applies WO- prefix for work orders
    - Applies INV- prefix for invoices

  3. Notes
    - No new tables or columns
    - No changes to any other logic in the trigger
    - Only the number generation lines are modified
*/
