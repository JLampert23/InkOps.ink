/*
  # Fix Chipply Import - Populate Individual Size Columns

  1. Changes
    - Updates the Chipply import processor to populate individual size columns
    - Maps each size from Chipply to its proper qty_* column
    - Supports youth sizes (YXS, YS, YM, YL, YXL), adult sizes (XS-5XL), and double sizes

  2. Size Mapping
    - Youth: YXS→qty_yxs, YS→qty_ys, YM→qty_ym, YL→qty_yl, YXL→qty_yxl
    - Adult: XS→qty_xs, S→qty_s, M→qty_m, L→qty_l, XL→qty_xl, 2XL→qty_2xl, etc.
    - Double: S/M→qty_sm, L/XL→qty_lxl, YS/YM→qty_ysym, YL/YXL→qty_ylyxl
*/
