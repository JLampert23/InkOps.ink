/*
  # Fix PO Auto-Creation to Use Individual Size Columns

  1. Problem
    - The `process_quote_approval()` trigger reads from `regular_sizes` JSONB column
    - But size quantities are stored in individual columns: qty_xs, qty_s, qty_m, qty_l, etc.
    - The `regular_sizes` JSONB is always empty `{}`
    - This caused all PO line items to fall into the "Mixed" fallback with no size breakdown

  2. Fix
    - Updates `process_quote_approval()` to build a sizes JSONB from individual qty_* columns
    - Uses `jsonb_strip_nulls()` to exclude sizes with zero quantity
    - Creates one PO line item per size (XS, S, M, L, XL, 2XL, 3XL, 4XL, 5XL, YXS, YS, YM, YL, YXL)
    - Also fixes garment_requirements_staging to use the computed sizes

  3. Tables Modified
    - No schema changes, only function logic updated

  4. Important Notes
    - Only affects future quote approvals
    - Existing POs with "Mixed" sizes are fixed separately below
*/
...
BEGIN
  IF NEW.status = 'approved' AND (OLD.status IS NULL OR OLD.status != 'approved') THEN
    ...
    LOOP
      ...
      FOR v_line_item IN
        SELECT item_number, brand, color, description,
      ...
      LOOP
        IF v_line_item.computed_supplier = v_supplier_group.resolved_name THEN
          v_computed_sizes := v_line_item.computed_sizes;

          IF v_computed_sizes IS NOT NULL AND v_computed_sizes != '{}'::jsonb
             AND (SELECT COUNT(*) FROM jsonb_each_text(v_computed_sizes)) > 0 THEN
            FOR v_size_key, v_size_qty IN
              SELECT key, value::text::integer
              FROM jsonb_each_text(v_computed_sizes)
