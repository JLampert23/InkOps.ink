/*
  # Fix Chipply Import - Create Imprint Per Component

  1. Changes
    - Modifies the Chipply import processor to create ONE imprint per COMPONENT (decoration)
    - Previously created only one imprint per process, even if process had multiple decorations
    - Each component now gets its own imprint with proper location and artwork

  2. What This Fixes
    - Process with multiple decorations (e.g., front logo + back text) now creates separate imprints
    - Each imprint has correct artwork URL and location from its component
    - Imprint numbers increment properly for each decoration
*/
...
BEGIN
  ...
  FOR i IN 0..(v_processes_count - 1) LOOP
    ...
    -- CHANGED: Loop through components and create one imprint per component
    ...
    IF v_components IS NOT NULL AND jsonb_typeof(v_components) = 'array' THEN
      ...
      IF v_components_count > 0 THEN
        FOR m IN 0..(v_components_count - 1) LOOP
          ...
          -- Extract artwork URL from this component
          v_artwork_url := NULL;
          BEGIN
          ...
          -- Create imprint for this component
          INSERT INTO quote_imprints (
            quote_id, company_id, imprint_number,
            type_of_work, details, artwork_url, 
            decoration_location, group_label, created_at
          ) VALUES (
            v_quote_id, v_company_id, v_imprint_number::text,
            v_artwork_name, v_comp_notes, v_artwork_url,
    ...
    -- Process products and create line items
