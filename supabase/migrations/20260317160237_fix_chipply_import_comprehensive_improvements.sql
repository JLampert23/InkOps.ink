/*
  # Fix Chipply Import Comprehensive Improvements

  1. Problems Fixed
    - Quantity field shows duplicate count (also stored in size breakdown)
    - Garment images not associated with imprint blocks
    - Nickname date format not human-readable

  2. Solutions
    - Set quantity to 0 (sizes already have the breakdown)
    - Collect garment images per group and add to imprints
    - Format dates as "Mon DD, YYYY" instead of "YYYY-MM-DD"

  3. Changes
    - Modified INSERT INTO quote_line_items to set quantity = 0
    - Added logic to collect and update garment_images on imprints
    - Changed TO_CHAR date format for nickname
*/
