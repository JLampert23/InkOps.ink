/*
  # Add billing queue entry when invoice is created from quote approval

  When a quote is approved, the trigger now also inserts the newly created
  invoice into the billing_queue table so it appears in the billing workflow.

  ## Changes
  - Added INSERT into billing_queue after invoice creation in process_quote_approval()
*/
...
BEGIN
  IF NEW.status = 'approved' AND (OLD.status IS NULL OR OLD.status != 'approved') THEN
    ...
    INSERT INTO work_orders (
      ...
      'draft', 'medium', NEW.production_due_date, NEW.customer_due_date, v_total_quantity, NEW.notes
