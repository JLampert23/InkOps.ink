...
export function parseGarmentFromPrintavoData(lineItem: any, description: string): ParsedGarmentData {
  ...
  if (lineItem.style) {
    const styleName = lineItem.style.name || '';
    const styleNumber = lineItem.style.number || '';
    if (styleName && styleNumber) {
      result.style = `${styleName} ${styleNumber}`;
    } else if (styleName || styleNumber) {
      result.style = styleName || styleNumber;
    }
  }

  if (!result.style && lineItem.product) {
    const styleName = lineItem.product.styleName || '';
    const styleNumber = lineItem.product.styleNumber || '';
    if (styleName && styleNumber) {
      result.style = `${styleName} ${styleNumber}`;
    } else if (styleName || styleNumber) {
      result.style = styleName || styleNumber;
  ...
  if (lineItem.color && lineItem.color.name) {
    result.color = lineItem.color.name;
  } else if (lineItem.product && lineItem.product.colorName) {
    result.color = lineItem.product.colorName;
  }

  if (lineItem.sizeQuantities && typeof lineItem.sizeQuantities === 'object') {
    for (const [size, qty] of Object.entries(lineItem.sizeQuantities)) {
      const quantity = Number(qty);
      if (quantity > 0) {
        result.sizes[size] = quantity;
      }
    }
  }

  if (!result.style || !result.color || Object.keys(result.sizes).length === 0) {
    const parsedFromDesc = parseGarmentDescription(description);
    if (!result.style) result.style = parsedFromDesc.style;
    if (!result.color) result.color = parsedFromDesc.color;
    if (Object.keys(result.sizes).length === 0) result.sizes = parsedFromDesc.sizes;
    if (!result.sku) result.sku = parsedFromDesc.sku;
  }
