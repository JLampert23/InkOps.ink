...
export function buildSanMarCdnFallbackUrl(style: string, _colorCode?: string): string[] {
  ...
  // Only use the generic style image — SanMar CDN does NOT use predictable
  // URL patterns for color variants. Color-specific images must come from the
  // Media Content API or the sanmar_image_map CDN cache.
  return [`${SANMAR_CDN_BASE}/${normalizedStyle}.jpg`];
}

export function buildSanMarCdnBackUrl(_style: string, _colorCode?: string): string {
  // Back images must come from the Media Content API, not guessed CDN URLs
  return '';
}
