...
async function syncInvoices(
  ...
  const recentInvoicesQuery = `
    query GetRecentInvoices($after: String, $first: Int = 3) {
      invoices(after: $after, first: $first, inProductionAfter: "2025-12-01T00:00:00Z", sortDescending: true) {
        edges {
          node {
            id
            visualId
  ...
  console.log(`Syncing invoices from Dec 2025+ with statuses: ${ALLOWED_STATUSES.join(', ')}...`);
